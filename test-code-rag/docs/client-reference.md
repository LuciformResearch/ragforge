...
Generated by RagForge – use this reference to drive both LLM agents and manual usage of the query builders.
...
## ⭐ Custom Methods (Generated from YAML Config)
...
Use `.llmRerank()` to intelligently rerank results — **no need to create a provider!**
...
- `withDefinedIn(depth?: number)` — Scope DEFINED_IN File
- `withConsumes(depth?: number)` — Scope CONSUMES Scope
- `withHasParent(depth?: number)` — Scope HAS_PARENT Scope
- `withUsesLibrary(depth?: number)` — Scope USES_LIBRARY ExternalLibrary
- `withInheritsFrom(depth?: number)` — Scope INHERITS_FROM Scope
