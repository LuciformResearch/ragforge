...
// const GRAVITY = new THREE.Vector3(0, -1, 0); // m/s^2
...
export class Car {
    ...
    public setGround(...objects: THREE.Object3D[]) {
       ...
       // console.log(`setGround called with ${objects.length} objects, ${this.groundObjects.length} valid`);
    }
    ...
    // Central raycast distance - shorter than wheel raycasts to allow takeoff
    private readonly CENTER_RAYCAST_DISTANCE = SUSPENSION_REST_LENGTH + 1.0;
    ...
    private updateSuspensionAndGroundConstraint(dt: number) {
       ...
       //     console.log(`TOUCH_THRESHOLD=${TOUCH_THRESHOLD.toFixed(2)}, distances: FL=${wheelHitInfos[0]?.distance?.toFixed(2)}, FR=${wheelHitInfos[1]?.distance?.toFixed(2)}, RL=${wheelHitInfos[2]?.distance?.toFixed(2)}, RR=${wheelHitInfos[3]?.distance?.toFixed(2)}`);
        ...
        if (this.carState === 'airborne') {
            ...
            // More aggressive when fewer wheels are actually touching
            if (hitWheels > 0) {
                const newUpVector = this._calculateChassisOrientation(wheelHitInfos);

                // Smooth the target orientation itself to reduce jitter from noisy raycasts
                ...
                // Base slerp speed based on how many wheels are ACTUALLY touching (close to ground)
                let baseSlerpSpeed: number;
                if (wheelsTouchingGround <= 1) {
                    baseSlerpSpeed = 0.3; // Aggressive - 0-1 wheels touching
                } else if (wheelsTouchingGround === 2) {
                    baseSlerpSpeed = 0.2; // Medium - 2 wheels
                } else if (wheelsTouchingGround === 3) {
                    baseSlerpSpeed = 0.15; // Gentle - 3 wheels
                } else {
                    baseSlerpSpeed = 0.08; // Very gentle - all 4 wheels touching (stable)
                }

                // Apply cubic ease-out curve: faster at start, slows down near target
                // This reduces jitter when close to the final orientation
                const currentQuat = this.rollPitchPivot.quaternion.clone();
                const angleDiff = currentQuat.angleTo(targetQuaternion);
                const maxAngleDiff = Math.PI / 4; // Reference angle for full speed

                // Normalize angle difference to 0-1 range, then apply cubic ease-out
                const t = Math.min(angleDiff / maxAngleDiff, 1.0);
                const easedT = 1 - Math.pow(1 - t, 3); // Cubic ease-out: starts fast, ends slow

                // Final slerp speed: base speed scaled by eased factor
                // Add a minimum to ensure we always make progress
                const slerpSpeed = Math.max(0.02, baseSlerpSpeed * easedT);

                this.rollPitchPivot.quaternion.slerp(targetQuaternion, slerpSpeed);
            }
        }
    }
    ...
    public update(dt: number) {
        ...
        if (Math.random() < 0.01) {
         //   console.log(`Car update: state=${this.state}, sens=${this.sens}, accel=${this.acceleration.toFixed(2)}, speed=${this.speed.toFixed(2)}, vel=(${this.velocity.x.toFixed(2)}, ${this.velocity.y.toFixed(2)}, ${this.velocity.z.toFixed(2)})`);
        }

        // 1. Reset net force and apply gravity only if not grounded
        this.netForce.set(0, 0, 0);
        if (!this.isGrounded) {
            this.netForce.add(GRAVITY.clone().multiplyScalar(this.mass));
        }

        // 2. Update inputs and calculate engine/brake forces
        this.updateInputsAndState(dt);
        this.updateForces(dt); // This adds engine/brake forces to netForce

        // 3. Integrate forces to update velocity and position
